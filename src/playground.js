...
function reducer(state, action) {
    let playerId =
        action.playerId || (action.player ? action.player.id : undefined)
    ...
    switch (action.type) {
        case "update_player":
            console.assert(
                playerId !== undefined,
                "Undefined playerId; required for:",
                action
            )
            return {
                ...state,
                players: { ...state.players, [playerId]: action.player },
            }
        case "remove_player":
            console.assert(
                playerId !== undefined,
        ...
        case "update_structure":
            return {
                ...
                structures: {
                    ...state.structures,
                    [action.structure.id]: action.structure,
                },
            }
        case "replace_state":
            return action.state
        ...
        case "remove_oldest_player_message":
            ...
            if (player) {
                ...
                return {
                    ...
                    players: {
                        ...state.players,
                        [playerId]: { ...player, visibleMessages },
                    },
                }
            } else {
                return state
        ...
        case "update_player_target":
            ...
            return {
                ...
                players: {
                    ...state.players,
                    [playerId]: { ...player, target: action.target },
                },
            }
...
function usePlayground() {
    ...
    const throttledEmit = useMemo(() => {
        return throttle(data =>
            socketRef.current.emit("event", data, TARGET_UPDATE_THROTTLE)
        )
    }, [])
...
function initSocketEventHandlers(socket, state, dispatch) {
    ...
    socket.on("disconnect", data => {
        ...
        handleServerEvent(
            { type: "player_exit", player: localPlayer },
            state,
            dispatch,
            socket
        )
...
function executeAction(action, socketRef, state, dispatch, throttledEmit) {
    ...
    console.assert(
        socketRef.current || action.type === "initialize",
        "tried executing action before initialization: ",
        action.type
    )
    ...
    switch (action.type) {
        case "initialize":
            ...
            break
        case "key_down":
            if (!RECOGNIZED_KEYS.includes(action.key)) return

            dispatch({
                type: "update_player_key_state",
                key: action.key,
                state: true,
...
function handleServerEvent(event, state, dispatch, socket) {
    ...
    switch (event.type) {
        ...
        case "full_state_update":
            dispatch({
                type: "replace_state",
                state: { ...event.state, localPlayerId },
            })
            break
        case "input_key_down":
        ...
        case "chat_message":
            ...
            const messageDisplayTime =
                5000 + 1000 * Math.floor(event.message.length / 100)
            dispatch({
            ...
            setTimeout(() => {
                dispatch({
                    type: "remove_oldest_player_message",
                    playerId: event.player.id,
                })
            }, messageDisplayTime)
        ...
        case "player_target_change":
            if (
                (event.playerId && !state.players[event.playerId]) ||
                event.playerId === state.localPlayerId
            ) {
                break
            }
            ...
            break
        case "update_player":
            if (event.player.id === localPlayerId) break
            dispatch({ type: "update_player", player: event.player })
            break
        case "update_structure":
            if (event.playerId === localPlayerId) break
