  ...
  useFrame(() => {
    ...
    if (domNodes.length === sceneNodes.length) {
      domNodes.forEach((dNode, i) => {
        ...
        const screenPoint = Util.vec3ToScreenPoint(
          worldPoint,
          camera,
          size.width,
          size.height,
...
const getGridSnapPoint = (e, partialPoints, cellSize) => {
  ...
  const modVec = new Vector2(
    Math.abs(Util.fract(uv.x / cellSize)) - 0.5,
    Math.abs(Util.fract(uv.y / cellSize)) - 0.5,
  );
  ...
  if (inRange) {
    ...
    const relWorldSnapPoint = new Vector3(
      x.dot(planePoint),
      y.dot(planePoint),
      z.dot(planePoint),
    );
